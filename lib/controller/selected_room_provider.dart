import 'dart:convert';
import 'package:chat_application/core/local_data/shared_prefs.dart';
import 'package:flutter/foundation.dart';
import 'package:flutter_riverpod/flutter_riverpod.dart';
import 'package:chat_application/fyrechat/fyrechat.dart' as fc;

final selectedRoomProvider =
    StateNotifierProvider<PersistentSelectedRoomNotifier, fc.Room?>((ref) {
  return PersistentSelectedRoomNotifier(ref);
});

class PersistentSelectedRoomNotifier extends StateNotifier<fc.Room?> {
  final Ref ref;
  ...
  Future<void> _loadPersistedData() async {
    try {
      ...
      if (jsonString.isNotEmpty) {
        final room = fc.Room.fromJson(jsonDecode(jsonString));
        state = room;
      }
      if (kDebugMode) {
  ...
  Future<void> setRoom(fc.Room? room) async {
    state = room;
    try {
      if (room == null) {
